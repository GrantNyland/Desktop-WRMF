//
//  UNIT      : Contains TFileReclamationPlantDatabaseAgent Class
//  AUTHOR    : Kholofelo Malokane(Cornastone)
//  DATE      : 03/06/2006
//  COPYRIGHT : Copyright © 2006 DWAF
//
//
unit UFileReclamationPlantDatabaseAgent;
...
type
  TFileReclamationPlantDatabaseAgent = class(TAbstractDatabaseAgent)
  protected
...
{ TFileReclamationPlantDatabaseAgent }

function TFileReclamationPlantDatabaseAgent.ReadUnkownDataSQL: string;
CONST OPNAME = 'TFileReclamationPlantDatabaseAgent.ReadUnkownDataSQL';
begin
  Result := '';
  try
    Result := 'SELECT Model,StudyAreaName,SubArea,Scenario,LineNumber,LineSection,LineData  '+
              ...
              ' AND FileType      =  1' +
              ' AND FileGroup     =  '+ IntToStr(fgReclamationPlantControl)+
              ' ORDER BY Model,StudyAreaName,SubArea,Scenario,LineNumber,LineSection';
     except on E: Exception do HandleError(E,OPNAME) end;
end;

function TFileReclamationPlantDatabaseAgent.WriteUnknownDataSQL: string;
CONST OPNAME = 'TFileReclamationPlantDatabaseAgent.WriteUnknownDataSQL';
begin
...
function TFileReclamationPlantDatabaseAgent.ReadModelDataFromDatabase(AFileName: TFileNameObject;
         ADataObject: TDataFileObjects;AProgressFunction: TProgressUpdateFuntion): boolean;
CONST OPNAME = 'TFileReclamationPlantDatabaseAgent.ReadModelDataFromDatabase';
var
  LMessage : String;
...
begin
  ...
  try
    ...
    LMessage := FAppModules.Language.GetString('TFileReclamationPlantDatabaseAgent.strReadStarted'); // Update
    ...
    try
      ...
      begin
        LReclamationPlantObject.HDextraLines.Add(Trim(LDataSet.DataSet.FieldByName('LineData').AsString));
        LDataSet.DataSet.Next;
      end;
      LDataSet.DataSet.Close;

      LMessage := FAppModules.Language.GetString('TFileReclamationPlantDatabaseAgent.strReadEnded');
      AProgressFunction(LMessage,ptNone,LStop);
      Result :=  True;
    finally
      LDataSet.Free;
...
function TFileReclamationPlantDatabaseAgent.WriteModelDataToDatabase(AFileName: TFileNameObject;
         ADataObject: TDataFileObjects;AProgressFunction: TProgressUpdateFuntion): boolean;
const OPNAME = 'TFileReclamationPlantDatabaseAgent.WriteModelDataToDatabase';
var
  LMessage   : string;
  LDataSet   : TAbstractModelDataset;
  LStop      : boolean;
...
begin
  ...
  try
     ...
     LMessage := FAppModules.Language.GetString('TFileReclamationPlantDatabaseAgent.strWriteStarted'); // Update
     AProgressFunction(LMessage,ptNone,LStop);
     ...
     LPlanningFileDataObject  := TPlanningFileDataObjects(ADataObject);
     LReclamationPlantObject  := LPlanningFileDataObject.ReclamationPlantFileDataObjects;
     if LReclamationPlantObject = nil then
       Exit;
     try
       FAppModules.Database.CreateDataset(integer(dtExecSQL), LDataSet);
         ...
         begin
           ...
           LDataSet.SetParams(['LineData'], [LReclamationPlantObject.HDextraLines[LCount]]);
           LDataSet.ExecSQL;
         end;
         LDataSet.DataSet.Close;
       ...
       if Result then
       begin
         LMessage := FAppModules.Language.GetString('TFileReclamationPlantDatabaseAgent.strWriteEnded');
         AProgressFunction(LMessage,ptNone,LStop);
       end;
       LDataSet.DataSet.Close;
     finally
...
function TFileReclamationPlantDatabaseAgent.ClearModelDataInDatabase(AFileName: TFileNameObject;
         AProgressFunction: TProgressUpdateFuntion; AQuetly: boolean): boolean;
const OPNAME = 'TFileReclamationPlantDatabaseAgent.ClearModelDataInDatabase';
var
  LMessage: string;
  LStop: boolean;
begin
