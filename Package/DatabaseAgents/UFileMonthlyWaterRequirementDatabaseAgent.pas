//
//  UNIT      : Contains TFileMonthlyWaterRequirementDatabaseAgent Class
//  AUTHOR    : Kholofelo Malokane(Cornastone)
//  DATE      : 03/06/2006
//  COPYRIGHT : Copyright © 2006 DWAF
//
//
unit UFileMonthlyWaterRequirementDatabaseAgent;
...
type
  TFileMonthlyWaterRequirementDatabaseAgent = class(TAbstractDatabaseAgent)
  protected
...
{ TFileMonthlyWaterRequirementDatabaseAgent }

function TFileMonthlyWaterRequirementDatabaseAgent.ReadUnkownDataSQL: string;
CONST OPNAME = 'TFileMonthlyWaterRequirementDatabaseAgent.ReadUnkownDataSQL';
begin
  Result := '';
  try
    Result := 'SELECT Model,StudyAreaName,SubArea,Scenario,LineNumber,LineSection,LineData  '+
              ...
              ' AND FileType      =  1' +
              ' AND FileGroup     =  '+ IntToStr(fgMonthlyWaterRequirement)+
              ' ORDER BY Model,StudyAreaName,SubArea,Scenario,LineNumber,LineSection';
     except on E: Exception do HandleError(E,OPNAME) end;
end;

function TFileMonthlyWaterRequirementDatabaseAgent.WriteUnknownDataSQL: string;
CONST OPNAME = 'TFileMonthlyWaterRequirementDatabaseAgent.WriteUnknownDataSQL';
begin
...
function TFileMonthlyWaterRequirementDatabaseAgent.ReadModelDataFromDatabase(AFileName: TFileNameObject;
         ADataObject: TDataFileObjects;AProgressFunction: TProgressUpdateFuntion): boolean;
CONST OPNAME = 'TFileMonthlyWaterRequirementDatabaseAgent.ReadModelDataFromDatabase';
var
  LMessage : String;
...
begin
  ...
  try
    ...
    LMessage := FAppModules.Language.GetString('TFileMonthlyWaterRequirementDatabaseAgent.strReadStarted'); // Update
    ...
    try
      ...
      begin
        LWaterRequirementObject.HDextraLines.Add(TrimRight(LDataSet.DataSet.FieldByName('LineData').AsString));
        LDataSet.DataSet.Next;
      end;
      LDataSet.DataSet.Close;

      LMessage := FAppModules.Language.GetString('TFileMonthlyWaterRequirementDatabaseAgent.strReadEnded');
      AProgressFunction(LMessage,ptNone,LStop);
      Result :=  True;
    finally
      LDataSet.Free;
...
function TFileMonthlyWaterRequirementDatabaseAgent.WriteModelDataToDatabase(AFileName: TFileNameObject;
         ADataObject: TDataFileObjects;AProgressFunction: TProgressUpdateFuntion): boolean;
const OPNAME = 'TFileMonthlyWaterRequirementDatabaseAgent.WriteModelDataToDatabase';
var
  LMessage   : string;
  LDataSet   : TAbstractModelDataset;
  LStop      : boolean;
...
begin
  ...
  try
     ...
     LMessage := FAppModules.Language.GetString('TFileMonthlyWaterRequirementDatabaseAgent.strWriteStarted'); // Update
     AProgressFunction(LMessage,ptNone,LStop);
     ...
     LPlanningFileDataObject  := TPlanningFileDataObjects(ADataObject);
     LWaterRequirementObject  := LPlanningFileDataObject.MonthlyWaterRequirementFileDataObjects;
     if LWaterRequirementObject = nil then
       Exit;
     try
       FAppModules.Database.CreateDataset(integer(dtExecSQL), LDataSet);
         ...
         begin
           ...
           LDataSet.SetParams(['LineData'], [LWaterRequirementObject.HDextraLines[LCount]]);
           LDataSet.ExecSQL;
         end;
         LDataSet.DataSet.Close;
       ...
       if Result then
       begin
         LMessage := FAppModules.Language.GetString('TFileMonthlyWaterRequirementDatabaseAgent.strWriteEnded');
         AProgressFunction(LMessage,ptNone,LStop);
       end;
       LDataSet.DataSet.Close;
     finally
...
function TFileMonthlyWaterRequirementDatabaseAgent.ClearModelDataInDatabase(AFileName: TFileNameObject;
         AProgressFunction: TProgressUpdateFuntion; AQuetly: boolean): boolean;
const OPNAME = 'TFileMonthlyWaterRequirementDatabaseAgent.ClearModelDataInDatabase';
var
  LMessage: string;
  LStop: boolean;
begin
