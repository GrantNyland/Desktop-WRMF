...
{$IFDEF MSWINDOWS}
uses system.HelpIntfs, vcl.WinHelpViewer, SysUtils, Windows, VCL.Forms, VCL.Controls;
{$ENDIF}
{$IFDEF LINUX}
...
begin
  ...
  SaveWinHelpTester := vcl.WinHelpViewer.WinHelpTester;
  vcl.WinHelpViewer.WinHelpTester := nil;
  Application.HelpCommand(HELP_CONTEXT, ContextID);
  vcl.WinHelpViewer.WinHelpTester := SaveWinHelpTester;
end;
...
begin
  ...
  if aEnable then begin
    if not Assigned(vcl.WinHelpViewer.WinHelpTester) then
      vcl.WinHelpViewer.WinHelpTester := TWinHelpTester.Create;
  end
  {Disable WinHelpTester - ie. Make HelpManager use WinHelpViewer again}
  else begin
    if Assigned(vcl.WinHelpViewer.WinHelpTester) then
      vcl.WinHelpViewer.WinHelpTester := Nil;
  end;
end;
...
initialization
  ...
  system.HelpIntfs.RegisterViewer(HelpViewer {ICustomHelpViewer}, HelpViewer.FHelpManager {IHelpManager});
