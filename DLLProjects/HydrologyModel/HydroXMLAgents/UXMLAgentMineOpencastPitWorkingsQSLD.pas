(******************************************************************************)
(* This unit contains the class TXMLAgentMineOpencastPitWorkingsQSLD.
(******************************************************************************)
unit UXMLAgentMineOpencastPitWorkingsQSLD;
...
type
  TXMLAgentMineOpencastPitWorkingsQSLD = class(TXMLAgent)
  protected
  public
...
procedure TXMLAgentMineOpencastPitWorkingsQSLD.PopulatePropertyLists;
const OPNAME = 'TXMLAgentMineOpencastPitWorkingsQSLD.PopulatePropertyLists';
begin
  try
    FAllProperties.CommaText   := 'StdDevWorkingsArea';
    FAllDescriptions.CommaText := '"Standard deviation workings area"';
  except on E: Exception do HandleError(E, OPNAME) end;
end;

procedure TXMLAgentMineOpencastPitWorkingsQSLD.AddSectionData (AModule    : INetworkModule;
                                                               ASectionNo : Integer;
                                                               ARootNode  : IXMLNode);
const OPNAME = 'TXMLAgentMineOpencastPitWorkingsQSLD.AddSectionData';
var
  LMine         : IMineModule;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        ARootNode.ChildNodes['Section'].Text   := 'MineOpencastPitWorkingsQSLD';

        LSectionNode := ARootNode.ChildNodes['MineOpencastPitWorkingsQSLD'];
        LSectionNode.ChildNodes['StdDevWorkingsArea'].Text := FloatToStr(LOpencastPit.StdDevWorkingsArea);
        LDataListNode := LSectionNode.ChildNodes['DataList'];
        for LIndex := 0 to LOpencastPit.WorkingsAreaQvsSLDNoOfPoints - 1 do
        begin
          LNode := LDataListNode.AddChild('Data');
          LNode.AddChild('Flow');
          LNode.AddChild('Load');
          LNode.ChildNodes['Flow'].Text := FloatToStr(LOpencastPit.WorkingsAreaQvsSLDFlowRefByIndex[LIndex]);
          LNode.ChildNodes['Load'].Text := FloatToStr(LOpencastPit.WorkingsAreaQvsSLDLoadByIndex[LIndex]);
        end;
      end;
    end;
  except on E: Exception do HandleError(E, OPNAME) end;
...
function TXMLAgentMineOpencastPitWorkingsQSLD.XSDText : String;
const OPNAME = 'TXMLAgentMineOpencastPitWorkingsQSLD.XSDText';
begin
  Result := '';
  try
    Result := GetXSD('MineOpencastPitWorkingsQSLD.xsd');
  except on E: Exception do HandleError(E, OPNAME) end;
end;

function TXMLAgentMineOpencastPitWorkingsQSLD.DoAdditionalValidation (AContext          : TValidationContext;
                                                                      APropertyName     : String;
                                                                      AFieldIndex       : String;
                                                                      AXSDDoc           : IXMLDocument;
                                                                      AXMLDocumentIn    : IXMLDocument;
...
const OPNAME = 'TXMLAgentMineOpencastPitWorkingsQSLD.DoAdditionalValidation';
var
  LResult         : Boolean;
  LSectionNode    : IXMLNode;
  LDataListNode   : IXMLNode;
...
begin
  ...
  try
    ...
    LSectionNode    := LRootNode.ChildNodes['MineOpencastPitWorkingsQSLD'];
    LDataListNode   := LSectionNode.ChildNodes['DataList'];
